...
# read data -----
## Distance data ----
distdata_dd <- read.csv("data/DistanceData/distdata_ddwholesample.csv",sep=";")
dim(distdata_dd)

distdata_ddoption2 <- read.csv("data/DistanceData/distdata_dd_todos2.csv",sep=";")
dim(distdata_ddoption2)

obsdata_dd <- read.csv("data/DistanceData/obsdata_dd.csv",sep=";")
dim(obsdata_dd)


distdata_lo <- read.csv("data/DistanceData/distdata_lowholesample.csv",sep=";")
dim(distdata_lo)


obsdata_lo <- read.csv("data/DistanceData/obsdata_lo.csv",sep=";")
dim(obsdata_lo)

segdata <-  read.csv("data/DistanceData/segdata.csv",sep=";")

preddata <-  read.csv("data/DistanceData/preddata.csv",sep=";")
...
# plot(st_geometry(patagonia))
# plot(st_geometry(patagonia), xlim=c(-63.6,-63.59), ylim=c(-42.5,-40.5), col="lightgrey", axes=TRUE)

####con archivos shp de Nadia
survey.area  <-  st_read("data/shp/survey.area.shp", quiet = TRUE)

# Verificar datos
print(survey.area)
plot(st_geometry(survey.area), col="light blue", axes=TRUE)

#este archivo ya esta recortado

#idem pred.polys
pred.polys <- st_read("data/shp/gridproy41.1.shp")
plot(st_geometry(pred.polys))
...
#######analisis exploratorio de datos de distancia
 ...
 hist(distdata_lo$distance, main = "Dusky dolphin n=119", breaks = 50, xlab = "Distance (m)")
