...
#   lo.dsm.xy.season.year   — count ~ s(x,y) + season + s(Ano)                         [primary]
...
#   lo.dsm.xy.season.year, lo.dsm.xy.fsyear.season, lo.dsm.xy.byyear.season,
#   pred.polys_m, survey.area_m, segdata, obsdata_lo_mod, trunc.dist_lo, target_crs
...
results_lo_fs <- lapply(seq_len(nrow(sy_combos)),
                        function(i) {
                          ...
                          vp <- dsm_var_gam(
                            dsm.obj   = lo.dsm.xy.fsyear.season,
                            pred.data = pred_grid,
                            off.set   = cell_area_m2
                          )
...
lo_abund_fs <- rbindlist(results_lo_fs) %>%
  mutate(seas = as.numeric(case_when(
    season == "Summer" ~ "0.1",
    season == "Fall" ~ "0.35",
    season == "Winter" ~ "0.60",
...
lo_abund_fs[, `:=`(season = as.character(season), year = as.numeric(year))]
lo_abund_fs <- merge(lo_abund_fs, obs_strata, by = c("season", "year"), all.x = TRUE)
lo_abund_fs[, season := factor(season, levels = season_levels)]
setorder(lo_abund_fs, sy)

##  abundance (fs) ----
### facets -----
p.lo.N.fs <- ggplot(lo_abund_fs, aes(x = year)) +
  geom_ribbon(aes(ymin = N_lo95, ymax = N_hi95), alpha = 0.2) +
  geom_line(aes(y = N_hat)) +
  geom_point(aes(y = N_hat, colour = "Density surface model"), size = 2) +
  geom_linerange(aes(ymin = N_obs_lo, ymax = N_obs_hi,
  ...
  facet_wrap(~ season, scales = "free_y") +
  labs(
    title    = "Dusky dolphin abundance by season and year (year-varying spatial)",
...
### continuous ----
p.lo.N.sy.fs <- ggplot(lo_abund_fs, aes(x = sy)) +
  geom_ribbon(aes(ymin = N_lo95, ymax = N_hi95), alpha = 0.2) +
  geom_line(aes(y = N_hat)) +
  geom_point(aes(y = N_hat, colour = "Density surface model"), size = 2) +
  geom_linerange(aes(ymin = N_obs_lo, ymax = N_obs_hi,
...
## density (fs) ----
### facets -----
p.lo.D.fs <- ggplot(lo_abund_fs, aes(x = year)) +
  geom_ribbon(aes(ymin = dens_lo, ymax = dens_hi), alpha = 0.2) +
  geom_line(aes(y = density)) +
  geom_point(aes(y = density,     colour = "Density surface model"), size = 2) +
  geom_linerange(aes(ymin = density_obs_lo, ymax = density_obs_hi,
  ...
  facet_wrap(~ season, scales = "free_y") +
  labs(
    title    = "Dusky dolphin density by season and year (year-varying spatial)",
    subtitle = "count ~ s(x, y, year_fac, bs = \"fs\") + season  |  ribbon = 95% CI (lognormal)",
    x        = "Year",
    y        = expression("Dolphins km"^{-2})
  ) +
...
### continuous ----
p.lo.D.sy.fs <- ggplot(lo_abund_fs, aes(x = sy)) +
  geom_ribbon(aes(ymin = dens_lo, ymax = dens_hi), alpha = 0.2) +
  geom_line(aes(y = density)) +
  geom_point(aes(y = density,     colour = "Density surface model"), size = 2) +
  geom_linerange(aes(ymin = density_obs_lo, ymax = density_obs_hi,
...
ply.lo.N.sy.fs <- plot_ly() %>%
  # 95% CI ribbon (lognormal)
  add_ribbons(data = lo_abund_fs,
              x = ~sy, ymin = ~N_lo95, ymax = ~N_hi95,
              color = I("black"), opacity = 0.2,
              line = list(color = 'transparent'),
              showlegend = FALSE) %>%
  # DSM line + dots
  add_trace(data = lo_abund_fs, x = ~sy, y = ~N_hat,
            type = 'scatter', mode = 'lines+markers',
            line = list(color = 'black', width = 1.5),
            marker = list(color = 'black', size = 6),
            name = 'Density surface model') %>%
  # HT linerange (invisible markers anchor error_y)
  add_trace(data = lo_abund_fs %>% filter(!is.na(N_obs)),
            x = ~sy, y = ~N_obs,
            type = 'scatter', mode = 'markers',
            marker = list(color = 'firebrick', size = 0, opacity = 0),
            error_y = list(type = "data", symmetric = FALSE,
...
ply.lo.N.sy.fs
## output (fs) ----
fwrite(lo_abund_fs, "output/DuskyDolphin/Abundance/LO_abundance_season_year_fsyear.csv")
ggsave(
  "output/DuskyDolphin/Abundance/LO_abundance_season_year_fsyear.png",
  plot   = p.lo.N.fs,
  width  = 13,
  height = 8
)
ggsave(
  "output/DuskyDolphin/Abundance/LO_density_season_year_fsyear.png",
  plot   = p.lo.D.fs,
  width  = 13,
  height = 8
)
ggsave(
  "output/DuskyDolphin/Abundance/LO_abundance_modelfit_fsyear.png",
  plot   = p.lo.N.sy.fs,
  width  = 13,
  height = 8
)
ggsave(
  "output/DuskyDolphin/Abundance/LO_density_modelfit_fsyear.png",
  plot   = p.lo.D.sy.fs,
  width  = 13,
  height = 8
)
...
lo_year_pe <- year_partial_effect(
  lo.dsm.xy.fsyear.season,
...
p.lo.year.pe <- ggplot(lo_year_pe, aes(year, partial)) +
  ...
  scale_x_continuous(breaks = 2006:2018) + #,
                     ...
                     # guide = guide_axis(minor.ticks = TRUE)) +
  labs(
    title    = "Dusky dolphin — partial effect of year (fs model)",
...
fwrite(lo_year_pe, "output/DuskyDolphin/Abundance/LO_year_partial_effect_fsyear.csv")
ggsave(
  "output/DuskyDolphin/Abundance/LO_year_partial_effect_fsyear.png",
  plot   = p.lo.year.pe,
  width  = 10,
  height = 6
)
...
results_lo_byyear <- lapply(seq_len(nrow(sy_combos)),
                            function(i) {
                              ...
                              vp <- dsm_var_gam(
                                dsm.obj   = lo.dsm.xy.byyear.season,
                                pred.data = pred_grid,
                                off.set   = cell_area_m2
                              )
...
lo_abund_byyear <- rbindlist(results_lo_byyear) %>%
  mutate(seas = as.numeric(case_when(
    season == "Summer" ~ "0.1",
    season == "Fall" ~ "0.35",
    season == "Winter" ~ "0.60",
...
# reuse the model-independent design-based estimate (obs_strata) from above
lo_abund_byyear[, `:=`(season = as.character(season), year = as.numeric(year))]
lo_abund_byyear <- merge(lo_abund_byyear, obs_strata, by = c("season", "year"), all.x = TRUE)
lo_abund_byyear[, season := factor(season, levels = season_levels)]
setorder(lo_abund_byyear, sy)

##  abundance (byyear) ----
### facets -----
p.lo.N.byyear <- ggplot(lo_abund_byyear, aes(x = year)) +
  geom_ribbon(aes(ymin = N_lo95, ymax = N_hi95), alpha = 0.2) +
  geom_line(aes(y = N_hat)) +
  geom_point(aes(y = N_hat, colour = "Density surface model"), size = 2) +
  geom_linerange(aes(ymin = N_obs_lo, ymax = N_obs_hi,
  ...
  facet_wrap(~ season, scales = "free_y") +
  labs(
    title    = "Dusky dolphin abundance by season and year (by-year spatial)",
...
### continuous ----
p.lo.N.sy.byyear <- ggplot(lo_abund_byyear, aes(x = sy)) +
  geom_ribbon(aes(ymin = N_lo95, ymax = N_hi95), alpha = 0.2) +
  geom_line(aes(y = N_hat)) +
  geom_point(aes(y = N_hat, colour = "Density surface model"), size = 2) +
  geom_linerange(aes(ymin = N_obs_lo, ymax = N_obs_hi,
...
## density (byyear) ----
### facets -----
p.lo.D.byyear <- ggplot(lo_abund_byyear, aes(x = year)) +
  geom_ribbon(aes(ymin = dens_lo, ymax = dens_hi), alpha = 0.2) +
  geom_line(aes(y = density)) +
  geom_point(aes(y = density,     colour = "Density surface model"), size = 2) +
  geom_linerange(aes(ymin = density_obs_lo, ymax = density_obs_hi,
  ...
  facet_wrap(~ season, scales = "free_y") +
  labs(
    title    = "Dusky dolphin density by season and year (by-year spatial)",
    subtitle = "count ~ s(x, y, by = year_fac) + year_fac + season  |  ribbon = 95% CI (lognormal)",
    x        = "Year",
    y        = expression("Dolphins km"^{-2})
  ) +
...
### continuous ----
p.lo.D.sy.byyear <- ggplot(lo_abund_byyear, aes(x = sy)) +
  geom_ribbon(aes(ymin = dens_lo, ymax = dens_hi), alpha = 0.2) +
  geom_line(aes(y = density)) +
  geom_point(aes(y = density,     colour = "Density surface model"), size = 2) +
  geom_linerange(aes(ymin = density_obs_lo, ymax = density_obs_hi,
...
ply.lo.N.sy.byyear <- plot_ly() %>%
  # 95% CI ribbon (lognormal)
  add_ribbons(data = lo_abund_byyear,
              x = ~sy, ymin = ~N_lo95, ymax = ~N_hi95,
              color = I("black"), opacity = 0.2,
              line = list(color = 'transparent'),
              showlegend = FALSE) %>%
  # DSM line + dots
  add_trace(data = lo_abund_byyear, x = ~sy, y = ~N_hat,
            type = 'scatter', mode = 'lines+markers',
            line = list(color = 'black', width = 1.5),
            marker = list(color = 'black', size = 6),
            name = 'Density surface model') %>%
  # HT linerange (invisible markers anchor error_y)
  add_trace(data = lo_abund_byyear %>% filter(!is.na(N_obs)),
            x = ~sy, y = ~N_obs,
            type = 'scatter', mode = 'markers',
            marker = list(color = 'firebrick', size = 0, opacity = 0),
            error_y = list(type = "data", symmetric = FALSE,
...
## output (byyear) ----
fwrite(lo_abund_byyear, "output/DuskyDolphin/Abundance/LO_abundance_season_year_byyear.csv")
ggsave(
  "output/DuskyDolphin/Abundance/LO_abundance_season_year_byyear.png",
  plot   = p.lo.N.byyear,
  width  = 13,
  height = 8
)
ggsave(
  "output/DuskyDolphin/Abundance/LO_density_season_year_byyear.png",
  plot   = p.lo.D.byyear,
  width  = 13,
  height = 8
)
ggsave(
  "output/DuskyDolphin/Abundance/LO_abundance_modelfit_byyear.png",
  plot   = p.lo.N.sy.byyear,
  width  = 13,
  height = 8
)
ggsave(
  "output/DuskyDolphin/Abundance/LO_density_modelfit_byyear.png",
  plot   = p.lo.D.sy.byyear,
  width  = 13,
  height = 8
)

# Soap-film shared surface ----
# lo.dsm.soap.season.year : count ~ s(x, y, bs = "so") + season + s(Ano)
#   A single spatial surface, edge-effect controlled (respects the gulf
#   boundary — see UTIL_DuskyDolphin_EdgeEffects.R / 4_DuskyDolphin_DSM_soap.R
...
results_lo_soap <- lapply(seq_len(nrow(sy_combos)),
                          function(i) {
                            ...
                            chk <- predict(lo.dsm.soap.season.year, newdata = pred_grid,
                            ...
                            if (anyNA(chk))
                              warning(sprintf(
                                paste0("season=%s, year=%d: %d/%d prediction grid cells fall ",
                                      ...
                                      "4_DuskyDolphin_DSM_soap.R and refit."),
                                s, a, sum(is.na(chk)), length(chk)))

                            vp <- dsm_var_gam(
                              dsm.obj   = lo.dsm.soap.season.year,
                              pred.data = pred_grid,
                              off.set   = cell_area_m2
                            )
...
lo_abund_soap <- rbindlist(results_lo_soap) %>%
  mutate(seas = as.numeric(case_when(
    season == "Summer" ~ "0.1",
    season == "Fall" ~ "0.35",
    season == "Winter" ~ "0.60",
...
# reuse the model-independent design-based estimate (obs_strata) from above
lo_abund_soap[, `:=`(season = as.character(season), year = as.numeric(year))]
lo_abund_soap <- merge(lo_abund_soap, obs_strata, by = c("season", "year"), all.x = TRUE)
lo_abund_soap[, season := factor(season, levels = season_levels)]
setorder(lo_abund_soap, sy)

##  abundance (soap) ----
### facets -----
p.lo.N.soap <- ggplot(lo_abund_soap, aes(x = year)) +
  geom_ribbon(aes(ymin = N_lo95, ymax = N_hi95), alpha = 0.2) +
  geom_line(aes(y = N_hat)) +
  geom_point(aes(y = N_hat, colour = "Density surface model"), size = 2) +
  geom_linerange(aes(ymin = N_obs_lo, ymax = N_obs_hi,
  ...
  facet_wrap(~ season, scales = "free_y") +
  labs(
    title    = "Dusky dolphin abundance by season and year (soap-film surface)",
...
### continuous ----
p.lo.N.sy.soap <- ggplot(lo_abund_soap, aes(x = sy)) +
  geom_ribbon(aes(ymin = N_lo95, ymax = N_hi95), alpha = 0.2) +
  geom_line(aes(y = N_hat)) +
  geom_point(aes(y = N_hat, colour = "Density surface model"), size = 2) +
  geom_linerange(aes(ymin = N_obs_lo, ymax = N_obs_hi,
...
## density (soap) ----
### facets -----
p.lo.D.soap <- ggplot(lo_abund_soap, aes(x = year)) +
  geom_ribbon(aes(ymin = dens_lo, ymax = dens_hi), alpha = 0.2) +
  geom_line(aes(y = density)) +
  geom_point(aes(y = density,     colour = "Density surface model"), size = 2) +
  geom_linerange(aes(ymin = density_obs_lo, ymax = density_obs_hi,
  ...
  facet_wrap(~ season, scales = "free_y") +
  labs(
    title    = "Dusky dolphin density by season and year (soap-film surface)",
    subtitle = "count ~ s(x, y, bs = \"so\") + season + s(Ano)  |  ribbon = 95% CI (lognormal)",
    x        = "Year",
    y        = expression("Dolphins km"^{-2})
  ) +
...
### continuous ----
p.lo.D.sy.soap <- ggplot(lo_abund_soap, aes(x = sy)) +
  geom_ribbon(aes(ymin = dens_lo, ymax = dens_hi), alpha = 0.2) +
  geom_line(aes(y = density)) +
  geom_point(aes(y = density,     colour = "Density surface model"), size = 2) +
  geom_linerange(aes(ymin = density_obs_lo, ymax = density_obs_hi,
...
ply.lo.N.sy.soap <- plot_ly() %>%
  # 95% CI ribbon (lognormal)
  add_ribbons(data = lo_abund_soap,
              x = ~sy, ymin = ~N_lo95, ymax = ~N_hi95,
              color = I("black"), opacity = 0.2,
              line = list(color = 'transparent'),
              showlegend = FALSE) %>%
  # DSM line + dots
  add_trace(data = lo_abund_soap, x = ~sy, y = ~N_hat,
            type = 'scatter', mode = 'lines+markers',
            line = list(color = 'black', width = 1.5),
            marker = list(color = 'black', size = 6),
            name = 'Density surface model') %>%
  # HT linerange (invisible markers anchor error_y)
  add_trace(data = lo_abund_soap %>% filter(!is.na(N_obs)),
            x = ~sy, y = ~N_obs,
            type = 'scatter', mode = 'markers',
            marker = list(color = 'firebrick', size = 0, opacity = 0),
            error_y = list(type = "data", symmetric = FALSE,
...
## output (soap) ----
fwrite(lo_abund_soap, "output/DuskyDolphin/Abundance/LO_abundance_season_year_soap.csv")
ggsave(
  "output/DuskyDolphin/Abundance/LO_abundance_season_year_soap.png",
  plot   = p.lo.N.soap,
  width  = 13,
  height = 8
)
ggsave(
  "output/DuskyDolphin/Abundance/LO_density_season_year_soap.png",
  plot   = p.lo.D.soap,
  width  = 13,
  height = 8
)
ggsave(
  "output/DuskyDolphin/Abundance/LO_abundance_modelfit_soap.png",
  plot   = p.lo.N.sy.soap,
  width  = 13,
  height = 8
)
ggsave(
  "output/DuskyDolphin/Abundance/LO_density_modelfit_soap.png",
  plot   = p.lo.D.sy.soap,
  width  = 13,
  height = 8
)
