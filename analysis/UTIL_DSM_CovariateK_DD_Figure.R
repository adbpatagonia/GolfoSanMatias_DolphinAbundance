...
#   * it plots the CSVs written by UTIL_DSM_CovariateK_DD.R, which is itself
#     a decision record for the reasons given in its own header. No refit
#     happens here, so the figure is reproducible from those stored CSVs --
#     but only from them, never from a fresh pipeline run.
#
