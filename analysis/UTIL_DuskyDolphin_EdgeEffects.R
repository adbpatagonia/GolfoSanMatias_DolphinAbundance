...
# 2026-07-07
#
# Edge-effect handling for dusky-dolphin density surface models.
# The fs per-year surfaces (lo.dsm.xy.fsyear.season) show year-specific boundary
...
#   segdata, obsdata_lo_mod, df.lo, trunc.dist_lo, distdata_lo_sf_m,
...
lo.map.density.year.fs.masked <- ggplot() +
  ...
  geom_sf(data = distdata_lo_sf_m %>% filter(Ano > 2006), aes(size = size), alpha = 0.7) +
  scale_fill_viridis_c(option = "turbo", name = expression("Dolphins km"^-2),
                       na.value = "grey95") +
  labs(title   = "Dusky dolphin (Spring) — fs per-year surface, edges masked",
...
lo.map.density.year.fs.masked

ggsave("output/DuskyDolphin/DSM/LO_DSM_Year_fsyear_masked.png",
       plot = lo.map.density.year.fs.masked, width = 13, height = 13)
...
# summary(lo.dsm.soap.season.year)
# appraise(lo.dsm.soap.season.year)
...
pred.polys_soap_m <- bind_rows(
  ...
  mutate(season = factor(season, levels = levels(obsdata_lo_mod$season)),
         Ano    = ref_ano)
...
lo.map.density.soap <- ggplot() +
  ...
  scale_fill_viridis_c(option = "turbo", name = expression("Dolphins km"^-2),
                       na.value = "grey95") +
  labs(title   = "Dusky dolphin — soap-film shared surface",
...
ggsave("output/DuskyDolphin/DSM/LO_DSM_Soap_Season.png",
       plot = lo.map.density.soap, width = 13, height = 13)
